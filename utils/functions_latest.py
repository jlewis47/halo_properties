...
def sum_sph(fields, r0, dilate_coords, ndilate, weights=None):
    ...
    if len(sh)==4:
        resampled = [(field*weight)[x,y,z].sum() / ndilate**3 for field,weight in zip(fields,weights)]
    else:
...
def ax_setup(axis,extremax=plt.xlim(),extremay=plt.ylim(),ylog=True,xlog=True):
    ...
    axis.tick_params(axis='both', which='both',directtion='in',right='on',top='on',pad=8)
...
def get_r200(M, omegam0=1.0):
    """
    Return r200 (in cell units) based on number of particles in a halo
    """
   
    return((M*3./800./np.pi/omegam0)**(1/3.))
