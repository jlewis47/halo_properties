...
def get_fof_suffix(fof_path):
    ...
    if len(find_ll[0])==0:return(fof_suffix)
...
def ll_to_fof_suffix(ll):
    ...
    if type(ll)==float:ll=int(ll*1000)
...
def get_suffix(fof_suffix='',rtwo_suffix='',frad_suffix=''):
...
def check_assoc_keys(assoc):
    ...
    assert np.sum(found)>0, 'error association method unrecognized, possible methods ares %s'%possible_keys
...
def divide_task_space(ldx, Nproc, rank):
    ...
    x,y,z = np.unravel_index(rank, list(map(lambda x : int(np.round(x)),[NperSide, NperSide, NperSide])))
