...
def catch_box(ctrs,test_pos,radius):
    """
    Return boolean array of [x,y,z]s in ctrs that are in a 2*radius box around test_pos
    
    """      
    
    x_cond = (ctrs[:,0]>=test_pos[0]-radius)*(ctrs[:,0]<=test_pos[0]+radius)
    
    y_cond = (ctrs[:,1]>=test_pos[1]-radius)*(ctrs[:,1]<=test_pos[1]+radius)
    
    z_cond =(ctrs[:,2]>=test_pos[2]-radius)*(ctrs[:,2]<=test_pos[2]+radius)
...
def mass_bin_stats(data,m,start=7.5,step=0.1,nb=50,count=False):

         #print(m)
         bins=np.asarray([(start+dm*step) for dm in range(nb)])
         #print(bins)
         #print([(bin_mass,(step+bin_mass)) for bin_mass in bins])
         if not count:
             mass_avgs = np.asarray([np.nanmean(data[np.all([m>10**bin_mass,m<10**(step+bin_mass)],axis=0)]) for bin_mass in bins])
             mass_meds = np.asarray([np.median(data[np.all([m>10**bin_mass,m<10**(step+bin_mass)],axis=0)]) for bin_mass in bins])             
             mass_stds = np.asarray([np.std(data[np.all([m>10**bin_mass,m<10**(step+bin_mass)],axis=0)]) for bin_mass in bins])
             mass_sums = np.asarray([np.sum(data[np.all([m>10**bin_mass,m<10**(step+bin_mass)],axis=0)]) for bin_mass in bins])
         bin_count = np.asarray([len(m[np.all([m>10**bin_mass,m<10**(step+bin_mass)],axis=0)]) for bin_mass in bins])
         #print(mass_stds,mass_avgs)
         #print(bins,mass_avgs,mass_stds)
         if not count:
             return(bins+step*0.5,mass_avgs,mass_meds,mass_stds,mass_sums,bin_count)
         else:
             return(bin_count)

def bin_stats_user(data,x_data,bins,count=False):

    """assumes regular steps -> don't give logspace normal space array !!!!!!"""

    step=bins[1]-bins[0]

    if not count:
        bin_data_avgs = np.asarray([np.nanmean(data[np.all([x_data>bin_data,x_data<(bin_data+step)],axis=0)]) for i_step,bin_data in enumerate(bins)])
        bin_data_meds = np.asarray([np.nanmedian(data[np.all([x_data>bin_data,x_data<(bin_data+step)],axis=0)]) for i_step,bin_data in enumerate(bins)])
        bin_data_stds =   np.asarray([np.nanstd(data[np.all([x_data>bin_data,x_data<(bin_data+step)],axis=0)]) for i_step,bin_data in enumerate(bins)])
        bin_data_sum =  np.asarray([np.nansum(data[np.all([x_data>bin_data,x_data<(bin_data+step)],axis=0)]) for i_step,bin_data in enumerate(bins)])
    bin_count = np.asarray([np.nansum(np.all([x_data>bin_data,x_data<(bin_data+step)],axis=0)) for i_step,bin_data in enumerate(bins)])
         #print(bin_data_nanstds,bin_data_avgs)
         #print(bins,bin_data_avgs,bin_data_nanstds)
    if not count:
        return(bin_data_avgs,bin_data_meds,bin_data_stds,bin_data_sum,bin_count)
    else:
        return(bin_count)

     
def bin_stats(data,m,start=7.5,step=0.1,nb=50):

         #print(m)
         bins=np.asarray([(start+dm*step) for dm in range(nb)])
         #print(bins)
         #print([(bin_mass,(step+bin_mass)) for bin_mass in bins])
         mass_avgs = np.asarray([np.nanmean(data[np.all([m>bin_mass,m<(step+bin_mass)],axis=0)]) for bin_mass in bins])
         mass_meds = np.asarray([np.nanmedian(data[np.all([m>bin_mass,m<(step+bin_mass)],axis=0)]) for bin_mass in bins])
         mass_stds = np.asarray([np.nanstd(data[np.all([m>bin_mass,m<(step+bin_mass)],axis=0)]) for bin_mass in bins])
         mass_sums = np.asarray([np.nansum(data[np.all([m>bin_mass,m<(step+bin_mass)],axis=0)]) for bin_mass in bins])
         bin_count = np.asarray([len(data[np.all([m>bin_mass,m<(step+bin_mass)],axis=0)]) for bin_mass in bins])
         #print(mass_nanstds,mass_avgs)
         #print(bins,mass_avgs,mass_nanstds)
         return(bins,mass_avgs,mass_meds,mass_stds,mass_sums,bin_count)
...
def get_arr(out_nb,tstep,path,info_path):


        (t,a,H0,om_m,om_b,unit_l,unit_d,unit_t,Lco,L,px_to_m,tstep) = get_infos(info_path,out_nb)

        redshift=1./a-1
        
        dats = np.asarray([tgt for tgt in os.listdir(path) if 'fesc_out_'+out_nb+'_' in tgt if os.path.isfile(os.path.join(path,tgt))])
        ...
        idxs = np.fromfile('/data2/jlewis/CoDa_ii_assoc/assoc_latest_halosID_'+out_nb,dtype=np.int64)
        ...
        with open('/data2/jlewis/CoDa_ii_assoc/Mp_%s' %out_nb, 'rb') as mass_file :
        ...
        mass_order = np.argsort(arr['M'])[::-1] #Backwards so descending
        arr=arr[mass_order]



        forgets = np.zeros(len(arr))

        '''
        Influence check
        -Order halos by descending mass
        -For each unmarked halo if size_fact*r200 radius contains a more massive star, hide this halo
        -Else hide all other halos in size_fact*r200
        -Check for stars too close to r200 (outside)
        '''
        ...
        size_fact=2.0
        halo_tree = spatial.cKDTree(get_pos(arr)*4096)

        #print(lone_stars[:10,-2])
        #print(lone_stars[0,1:4])

        #print(lone_stars[lone_stars[:,-2]<=10,-2])

        # # unique halo stars
        yng_stars_wh = (stars[:,-2]<=10+tstep)*(stars[:,-2]>0)
        yng_stars = stars[yng_stars_wh]
        unique_star_idxs,unique_star_idxs_arg = np.unique(star_idxs[yng_stars_wh],return_index=True)
        yng_unique_stars = yng_stars[unique_star_idxs_arg]


        yng_lone_stars = lone_stars[(lone_stars[:,-2]<=10+tstep)*(lone_stars[:,-2]>0)]
        #print(len(yng_stars))
        star_tree = spatial.cKDTree(yng_unique_stars[:,1:4]*4096)

        lone_star_tree = spatial.cKDTree(yng_lone_stars[:,1:4]*4096)


        ctrs = do_half_round(get_pos(arr)*4096)




        print('Halo neighbour checking')
        for halo_nb,halo in enumerate(arr):

            #print('Halo nb %d out of %d' %(halo_nb,len(arr)))

            if  halo['SFR10']==0 or forgets[halo_nb] ==1:
                 continue

            radius = get_r200(halo['M']/Mp)#halo[-1]#fof_tab[halo_nb,-1]
            #print(radius,get_r200(halo[3]/Mp))

            ctr = ctrs[halo_nb]

            nghbrs = np.asarray(halo_tree.query_ball_point(ctr,radius*size_fact))
            

            if np.any(arr['M'][nghbrs]>halo['M']):
                forgets[halo_nb]=1
                #if halo['M']<1e9 : plt.scatter(0,np.log10(halo['M']), facecolors='r',edgecolors='k')

            elif len(nghbrs) > 1 :
                forgets[np.asarray([nghbr for nghbr in nghbrs if nghbr != halo_nb])]=1

        print(np.sum(forgets))
        
        forgets_mult_det = np.copy(forgets)
        
        forgets[(arr['M']<dw_mass)]=1
        ...
        forgets[arr['SFR10']==0]=1

        forgets_SFR=np.copy(forgets)
        
        #forgets[arr['debug_surfaces']<5]=1
        #print(np.count_nonzero(arr['debug_surfaces']<5))
        ...
        print('Stellar neighbour checking')

        for halo_nb,halo in enumerate(arr):

            #print('Halo nb %d out of %d' %(halo_nb,len(arr)))

            if forgets[halo_nb] == 1 or halo['SFR10']==0:
                 continue

            radius = get_r200(halo['M']/Mp)#fof_tab[halo_nb,-1]
            ...
            #Outside exclude
            out_lim = radius+3
            nghbr_stars = yng_unique_stars[star_tree.query_ball_point(ctr,out_lim)]
            
            stars = yng_unique_stars[star_tree.query_ball_point(ctr,radius)]

            lone_nghbr_stars = yng_lone_stars[lone_star_tree.query_ball_point(ctr,out_lim)]


            mass_outside_stars=np.sum(lone_nghbr_stars[:,0])+np.sum(nghbr_stars[:,0])
            mass_in = np.sum(stars[:,0])
            
            if (mass_outside_stars-mass_in)/mass_in>0.4: #1
                forgets[halo_nb]=1
                continue

            

        # # Apply forgetting to halos
        arr=np.copy(arr[forgets==0])

    
        return(forgets_SFR,forgets_mult_det,tab,forgets)

        
def write_arr(out_nb,key_word,tstep,path,info_path):

    forgets_SFR,forgets_mult_det,tab,forgets=get_arr(out_nb,tstep,path,info_path)

    with open(os.path.join(path,'filtrd_tab_%s_%s'%(key_word,out_nb)),'wb') as dest :
        np.save(dest,len(tab))
        np.save(dest,len(tab.dtype.names))

        np.save(dest,tab)
        
    with open(os.path.join(path,'filtrd_forgets_SFR_%s_%s'%(key_word,out_nb)),'wb') as dest :
        np.save(dest,len(forgets_SFR))
        np.save(dest,forgets_SFR)
        
    with open(os.path.join(path,'filtrd_forgets_%s_%s'%(key_word,out_nb)),'wb') as dest :
        np.save(dest,len(forgets))
        np.save(dest,forgets)

    with open(os.path.join(path,'filtrd_forgets_mult_det_%s_%s'%(key_word,out_nb)),'wb') as dest :
        np.save(dest,len(forgets_mult_det))
        np.save(dest,forgets_mult_det)
...
def get_box_sum(active_star,to_sum,recentre,wbox):

    #print(active_star,recentre)
    star_ctr = np.int16(np.floor(active_star[1:4]-recentre))
    #print(star_ctr,np.shape(wbox))
    over_step=(star_ctr==np.shape(wbox))
    under_step=(star_ctr==[0,0,0])
    if np.any(over_step) :
        star_ctr[over_step]-=1
    if np.any(under_step) :
        star_ctr[under_step]+=1

        
    wbox[star_ctr[2],star_ctr[1],star_ctr[0]]+=to_sum
    return(wbox,[star_ctr[2],star_ctr[1],star_ctr[0]])
