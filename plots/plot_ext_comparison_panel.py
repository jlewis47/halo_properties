...
def load_mags(sim_name, out_nb, assoc_mthd, ll, rtwo_fact, ext_keys):


    print(f'LOADING : {out_nb:d} ll={ll:f}, {rtwo_fact:f}xr200, association: {assoc_mthd:s}, {ext_keys[0]:s}')       
...
for iplot, out_nb in enumerate(out_nbs):
    ...
    for assoc_mthd, ll, r200 in zip(assoc_mthds, lls, r200s):
        ...
        for ext_key, mag_ext_key in zip(ext_keys, mag_ext_keys):
            ...
            out_file = os.path.join(out_path, f'exts_{out_nb:d}_{assoc_mthd:s}_{ll:.2f}_{r200:.1f}_{ext_key:s}')
            ...
            if overwrite or not exists:
                ...
                mags = load_mags("CoDaIII", out_nb, assoc_mthd, ll, r200, [mag_ext_key, "mag_no_dust"])
            ...
            line = plot_ext(fig, ax, bins+2.5*np.log10(1./0.8), ext, linewidth=3, elinewidth=1, capsize=3)
            label = mag_ext_key
