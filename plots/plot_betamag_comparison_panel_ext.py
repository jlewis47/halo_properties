...
from plot_functions.UV.UVslope.betaVSmag import make_magbeta, plot_magbeta, plot_magbeta_constraints, plot_dustier_magbeta
...
def load_magbeta(sim_name, out_nb, assoc_mthd, ll, rtwo_fact, ext_key):
...
for iplot, out_nb in enumerate(out_nbs):
    ...
    for assoc_mthd, ll, r200 in zip(assoc_mthds, lls, r200s):
        ...
        for ext_key, mag_ext_key in zip(ext_keys, mag_ext_keys):
            ...
            out_file = os.path.join(out_path, f'magbeta_{out_nb:d}_{assoc_mthd:s}_{ll:.2f}_{r200:.1f}_{ext_key:s}')
            ...
            if overwrite or not exists:
                ...
                mags,betas = load_magbeta("CoDaIII", out_nb, assoc_mthd, ll, r200, ext_key)
            ...
            line = plot_magbeta(fig, ax, bins + 2.5*np.log10(1./0.8), rel, linewidth=3)
            label = mag_ext_key
    ...
    dustier_line, dustier_label = plot_dustier_magbeta(ax, redshift, zprec=0.1, color='k')
    ...
    ax.legend(lines+obs_lines+dustier_line, labels+obs_labels+dustier_label, framealpha=0.0, title=f"z={redshift:.1f}")
