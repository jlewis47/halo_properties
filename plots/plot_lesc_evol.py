...
for out_nb in out_nbs:
    ...
    out_file = os.path.join(out_path, f'lescs_{redshift:.1f}_{stat_mthd:s}_{assoc_mthd:s}_{ll:.2f}_{r200:.1f}_{fesc_type:s}')

    exists = os.path.isfile(out_file)

    if overwrite or not exists:

        mass, fesc, lintr, sfing = load_data("CoDaIII", out_nb, assoc_mthd, ll, r200, fesc_type=fesc_type, xkey='mass')
        xbins, lesc = xy_stat(mass, fesc*lintr, xbins = mass_bins, mthd=stat_mthd)
        xbins, lesc_sfing = xy_stat(mass[sfing], (fesc*lintr)[sfing], xbins = mass_bins, mthd=stat_mthd)
    

        # if not exists :
        with h5py.File(out_file, 'w') as dest:
            dest.create_dataset("xbins", data = xbins, dtype='f4', compression='lzf')
            dest.create_dataset("%s_lescs"%stat_mthd, data = lesc, dtype='f8', compression='lzf')
            dest.create_dataset("%s_lescs_sfing"%stat_mthd, data = lesc_sfing, dtype='f8', compression='lzf')
...
for z, mass, lesc, lesc_sfing in zip(redshifts, masses, lescs, lescs_sfing):
    ...
    lesc_Mh_plot(fig, ax, mass, lesc_sfing, fesc_type, redshift=None, ls='--', c=lines[-1].get_color())
