...
for iplot, out_nb in enumerate(out_nbs):
    ...
    for assoc_mthd, ll, r200 in zip(assoc_mthds, lls, r200s):
        ...
        for ext_key, mag_ext_key in zip(ext_keys, mag_ext_keys):
            ...
            if line==None:
                line = uvlf_plot(fig, ax, bins + 0.25, uvlf, linewidth=3, elinewidth=1, capsize=3, marker='none')
                area = ax.fill_between(bins + 0.25, uvlf-err, uvlf+err, alpha=0.5, color=line.get_color())
                lines+=[(line,area)]
                labels+=["CoDa III"]
            else:
                uvlf_plot(fig, ax, bins + 0.25, uvlf, linewidth=3, elinewidth=1, capsize=3, marker='none', ls='--', color=line.get_color())
...
for iax,(zed,ax) in enumerate(zip(redshifts,axs)):
    ...
    fig.savefig('figs/uvlf_prod_%.1f.png'%zed, bbox_inches=extent.expanded(1.275, 1.275))   
    fig.savefig('figs/uvlf_prod_%.1f.pdf'%zed, bbox_inches=extent.expanded(1.275, 1.275), format='pdf')   
