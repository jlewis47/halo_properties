...
fig, axs = plt.subplots(nrows, ncols, sharex=True, sharey=True, figsize=(27,12), dpi=200)
...
for iplot, out_nb in enumerate(out_nbs):
    ...
    for assoc_mthd, ll, r200 in zip(assoc_mthds, lls, r200s):
        ...
        for ext_key, mag_ext_key in zip(ext_keys, mag_ext_keys):
            ...
            out_file = os.path.join(out_path, f'smfs_{out_nb:d}_{assoc_mthd:s}_{ll:.2f}_{r200:.1f}_{ext_key:s}')
            ...
            if overwrite or not exists:
                ...
                stmasses = load_stellar_masses("CoDaIII", out_nb, assoc_mthd, ll, r200)
            ...
            if line==None:
                line = smf_plot(fig, ax, bins, smf, linewidth=3, elinewidth=1, capsize=3, marker=None)
                area = ax.fill_between(bins, smf-err, smf+err, alpha=0.5, color=line.get_color())
                lines+=[(line,area)]
                labels+=["CoDa III"]
            else:
                smf_plot(fig, ax, bins, smf, linewidth=3, elinewidth=1, capsize=3, marker=None, ls='--', color=line.get_color())
    

            
            if iplot%ncols!=0:ax.set_ylabel('')
            if iplot/ncols<1 and nrows>1:ax.set_xlabel('')
    ...
    leg = ax.legend(lines+obs_lines, labels+obs_labels, framealpha=0.0, title=f"z={redshift:.1f}", 
    prop={"size":14}, loc='lower left')
...
for iax,(zed,ax) in enumerate(zip(redshifts,axs)):
    ...
    ax.tick_params(which='major', direction='in', labelleft=True, labelbottom=True, size=15)
    ...
    fig.savefig('figs/smf_prod_%.1f.png'%zed, bbox_inches=extent.expanded(1.275, 1.275))   
    fig.savefig('figs/smf_prod_%.1f.pdf'%zed, bbox_inches=extent.expanded(1.275, 1.275), format='pdf')   
