...
def linr(x,a,b):return(a * x + b)

#att coef type
class att_coefs:

    def __init__(self, name, low, high, tionising, sixteen=None):

        self.name = name

        self.Kappa912 = tionising
        self.Kappa1500 = low
        self.Kappa2500 = high
...
def att_coef_draine_file(path):
    ...
    return(att_coefs(path.split('/')[-1],
                     kappa_wvlngth(tab,1500.),
                     kappa_wvlngth(tab,2500.),
                     kappa_wvlngth(tab,912.),
                     sixteen=kappa_wvlngth(tab,1600.)))