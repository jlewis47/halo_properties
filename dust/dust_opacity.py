...
def ray3D(X0,Y0,Z0,D_lim,phi,theta,lim):
    ...
    #need to find directtion in x,y,y based on phi
...
def shoot_star_path(pos,dist_obs,dust,opacity,rlim,px_to_m,ldx):
    ...
    xtgt=-0-xstt
    ytgt=-0-ystt
    ztgt=-dist_obs-zstt #our maps are in (x,y) plane so we want extincttion in perp directtion
...
# 
    # if np.any(np.asarray([Xs,Ys,Zs])<0):
        # print(pos,rlim,dust.shape, ldx)
        # print(Zs[0],Zs[-1],D_tot)
...
def shoot_star_path_cheap(pos,dust,rlim):
...
def shoot_star_path_cheap_multid(pos,dust,rlim):
    ...
    Zs=np.arange(zstt,min(zstt+rlim,dust.shape[1]),1,dtype=np.int16)
    Ys=np.ones_like(Zs,dtype=np.int16)*int(ystt)
    Xs=np.ones_like(Zs,dtype=np.int16)*int(xstt)
...
def shoot_star_path_cheap_mean(pos,dust,rlim):
