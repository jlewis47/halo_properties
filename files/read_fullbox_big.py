...
def o_fullbox_big(data_pth, name, subdim=512, dim=512, subcube_nb=0):
    ...
    # print(max(datas_nb))
    sub_data_nb = np.int32(
        np.ones((square_nb)) * line_nb * (subcube_nb)
        + 1
        + (line_nb - 1) * line_nb * 8 * (subcube_nb // 1)
        + (height_nb - 1) * (square_nb) * 64 * (subcube_nb // 64)
    )
    # print(sub_data_nb)
    sub_data_nb = np.asarray(
        [elem + ind % line_nb for ind, elem in enumerate(sub_data_nb)]
    )
    # print(sub_data_nb)
    sub_data_nb = np.asarray(
        [elem + (ind // line_nb) * 8 for ind, elem in enumerate(sub_data_nb)]
    )
    # print(sub_data_nb)
    sub_data_nb = np.tile(sub_data_nb, height_nb)
    # print(sub_data_nb)
    sub_data_nb = [
        int(elem + (ind // square_nb) * (8) ** 2)
        for ind, elem in enumerate(sub_data_nb)
    ]
    # print(sub_data_nb)
    ...
    # print(sub_data_nb)
    datas = [
        datas[np.argwhere(val == datas_nb)[0]][0]
        for val in map(lambda x: x - 1, sub_data_nb)
    ]
...
# '''test'''
# xtion_test = o_fullbox_big('/data2/CoDaII/reduced/output_00088/fullboxfullres/','rho',512,0)

# plt.imshow(np.log10(xtion_test[0,:,:]))
# #plt.figure()
# #plt.imshow(xtion_test[:,0,:])
# #plt.figure()
# #plt.imshow(xtion_test[:,:,0])
# plt.show()
