'''
This verstion doesn't use mass attenuattion coefs and dust masses but does as WU+2020

This ONLY computes magnitudes and their extincttion

Metal column densities give an extincttion value for a LoS using a MW type law
This extincttion is then scaled by a relavent factor (LoS DTM VS MW DTM)
...
def watson11_Av_law(nZ):
        ...
        """
        Takes metal column density (cm^-2) and turns it into an extincttion
...
def compute_AV_ext(out_nb,ldx,path,sim_name):
        ...
        #(0,0,0) px locattion of 512**3 cube within whole data set
        ...
        #This bit makes an array dilate_fact times bigger than the RAMSES data and fills using the pertionicity of the data that way we don't care about being close to the edge with r200 (memory inefficient but fast, only works for one big chuck .... best use with ldx<=1024)
        ...
        for ind,halo in enumerate(phew_tab):
            ...
            if phew_star_nb[ind]!=0 :
                    ...
                    if  np.shape(emissivity_box)==1:
                        ...
                        #instead of doing star per star for fesc, fesc_dust, extincttion, we loop over halo cells with stellar particles
