...
def write_fields(out_file, rho, rhod, xion, temp, age, Z, mag, beta, fesc, mass, lintr, att_sets
    ...
    with h5py.File(out_file, "w") as out_halos:
        dset=out_halos.create_dataset("gas density", data=rho, dtype='f4', compression="lzf")
        dset.attrs["unit"]="g.cm^-3"
    
        dset=out_halos.create_dataset("dust density", data=rhod, dtype='f4', compression="lzf")
        dset.attrs["unit"]="g.cm^-3"
    
        dset=out_halos.create_dataset("gas xion", data=xion, dtype='f4', compression="lzf")
        dset.attrs["unit"]=""
        dset.attrs["descrption"]="fraction of ionised gas"
    
        dset=out_halos.create_dataset("gas temperature", data=temp, dtype='f4', compression="lzf")
        dset.attrs["unit"]="K"
    
        dset=out_halos.create_dataset("stellar mass", data=mass, dtype='f4', compression="lzf")
        dset.attrs["unit"]="Msun"

        dset=out_halos.create_dataset("halo lintr", data=lintr, dtype='f8', compression="lzf")
        dset.attrs["unit"]="s^-1"

        dset=out_halos.create_dataset("stellar age", data=age, dtype='f4', compression="lzf")
        dset.attrs["unit"]="Myr"
    
        dset=out_halos.create_dataset("stellar Z", data=Z, dtype='f4', compression="lzf")
        dset.attrs["unit"]="absolute metallicity"


        for iset in range(nsets):
            
                dset=out_halos.create_dataset("stellar mag %s"%att_sets[iset].name, data=mag[iset], dtype='f4', compression="lzf")
                dset.attrs["unit"]=""
                dset.attrs["description"]="AB magnitude at 1600 Ang (extinction 1D, LoS direction = 0)"
            
                dset=out_halos.create_dataset("stellar beta %s"%att_sets[iset].name, data=beta[iset], dtype='f4', compression="lzf")
                dset.attrs["unit"]=""
                dset.attrs["description"]="UV slope (reddening 1D, LoS direction = 0)"
            
                dset=out_halos.create_dataset("stellar fesc %s"%att_sets[iset].name, data=fesc[iset], dtype='f8', compression="lzf")
                dset.attrs["unit"]="g.cm^-3"
                dset.attrs["description"]="ray-traced fraction of escaping photons that reach IGM"




def compute_part_pties(
    out_nb, overwrite=False, rtwo_fact=1, ll=0.2, assoc_mthd="", test=False, mbin=1e11, mbin_width=1e10
):
    ...
    for z_subnb in range(subs_per_side):
        for y_subnb in range(subs_per_side):
            for x_subnb in range(subs_per_side):
                ...
                out_file = os.path.join(analy_out, f"stellar_pties_mbin{mbinstr:s}_{subcube_nb:d}")
                ...
                for ind, halo in enumerate(sub_halo_tab):
                    ...
                    if sub_halo_star_nb[ind] > 0:
                        ...
                        if test: print(f'Halo {ind:d} has {nb_cur_stars:d} stars')
                        ...
                        for icell, (
                            ...
                            for iset in range(len(att_sets)):
                                tau_box[iset, x_cell, y_cell, z_cell] += (
                                    sum_over_rays_bias(
                                        sm_taus[iset],
                                        sm_ctr,
                                        r_px, 
                        ...
                        if test:
                            ...
                            # plt.colorbar()
                            img=ax[0,1].imshow((tau_box[0, :, :, int(smldx / 2)]), norm=LogNorm(vmin=vmin, vmax=vmax))
                            plt.colorbar(img)
                            img=ax[0,2].imshow((sm_rho[:, :, int(smldx / 2)] * unit_d), norm=LogNorm(vmin=1e-28,vmax=1e-24))
                            plt.colorbar(img)
                            img=ax[0,3].imshow(np.exp(-sm_taus[0, :, :, int(smldx / 2)]), norm=LogNorm(vmin=vmin, vmax=vmax))
                            plt.colorbar(img)
                            ax[0,4].imshow((normed_emissivity_box[:, :, int(smldx / 2)]))
                            


                            ax[1,0].imshow(sm_taus[0, :, int(smldx / 2), :])
                            # plt.colorbar()
                            img=ax[1,1].imshow((tau_box[0, :, int(smldx / 2), :]), norm=LogNorm(vmin=vmin, vmax=vmax))
                            plt.colorbar(img)
                            img=ax[1,2].imshow((sm_rho[:, int(smldx / 2), :] * unit_d), norm=LogNorm(vmin=1e-28,vmax=1e-24))
                            plt.colorbar(img)
                            img=ax[1,3].imshow(np.exp(-sm_taus[0, :, int(smldx / 2), :]), norm=LogNorm(vmin=vmin, vmax=vmax))
                            plt.colorbar(img)
                            ax[1,4].imshow((normed_emissivity_box[:, int(smldx / 2), :]))

                            ax[2,0].imshow(sm_taus[0, int(smldx / 2), :, :])
                            # plt.colorbar()
                            img=ax[2,1].imshow((tau_box[0, int(smldx / 2), :, :]), norm=LogNorm(vmin=vmin, vmax=vmax))
                            plt.colorbar(img)
                            img=ax[2,2].imshow((sm_rho[int(smldx / 2), :, :] * unit_d), norm=LogNorm(vmin=1e-28,vmax=1e-24))
                            plt.colorbar(img)
                            img=ax[2,3].imshow(np.exp(-sm_taus[0, int(smldx / 2), :, :]), norm=LogNorm(vmin=vmin, vmax=vmax))
                            plt.colorbar(img)
                            ax[2,4].imshow((normed_emissivity_box[int(smldx / 2), :, :]))
                            ...
                            circle = plt.Circle((smldx/2, smldx/2), r_px, color='r', fill=False)

                            fig.savefig('test_tau_VS_ray')


                        print(star_taus_fesc.min(), star_taus_fesc.max(), star_taus_fesc.mean())

                        # print(np.argmax(tau_box[0]), np.argmax(normed_emissivity_box))

                        stellar_masses[ind_star:ind_star + nb_cur_stars] = cur_stars["mass"] #missing /(1 - eta_sn) here...


                        # print(np.sum(stellar_masses[ind_star:ind_star + nb_cur_stars]), sub_halo_tab["mstar"][ind])

                        stellar_rho[ind_star:ind_star + nb_cur_stars] = sm_rho[
                            star_sm_posz, star_sm_posy, star_sm_posx
                        ] * unit_d #g.cm^-3
                        stellar_rhod[ind_star:ind_star + nb_cur_stars] = sm_rhod[
                            star_sm_posz, star_sm_posy, star_sm_posx
                        ] * unit_d #g.cm^-3
                        stellar_xion[ind_star:ind_star + nb_cur_stars] = sm_xion[
                            star_sm_posz, star_sm_posy, star_sm_posx
                        ]
                        stellar_taus[ind_star:ind_star + nb_cur_stars] = sm_taus[0, 
                            star_sm_posz, star_sm_posy, star_sm_posx
                        ]                        
                        stellar_Z[ind_star:ind_star + nb_cur_stars] = cur_stars["Z/0.02"] * 0.02
                        stellar_age[ind_star:ind_star + nb_cur_stars] = cur_stars["age"]
                        ...
                        for iset, att_set in enumerate(att_sets):
                            ...
                            stellar_lintr[ind_star:ind_star + nb_cur_stars] = cur_star_luminosity #s^-1

                            stellar_mag[iset, ind_star:ind_star + nb_cur_stars] = -2.5 * np.log10(
                                halo_fluxes * star_trans
                            )

                            stellar_beta[iset, ind_star:ind_star + nb_cur_stars] = comp_betas_indv(
                            ...
                            stellar_fesc[iset, ind_star:ind_star + nb_cur_stars] = tau_box[
                            0, star_sm_posz, star_sm_posy, star_sm_posx
                        ]

                        halo_mag = -2.5*np.log10(np.sum(halo_fluxes))
                        halo_mag_ext = -2.5*np.log10(np.sum(halo_fluxes * np.exp(
                                    -star_taus[iset]
                                    * att_set.Kappa1600
                                    / att_set.Kappa912
                                    )))
                        ...
                        delta_lambda = np.log10(2500./1500.)

                        halo_beta = np.log10(np.sum(high_conts*cur_stars['mass']/(1-eta_sn))/
                        np.sum(low_conts*cur_stars['mass']/(1-eta_sn)))/delta_lambda
                        halo_beta_ext = np.log10(np.sum(high_conts*cur_stars['mass']/(1-eta_sn)*np.exp(-star_taus[4] * wv_convert_high))/
                        np.sum(low_conts*cur_stars['mass']/(1-eta_sn)*np.exp(-star_taus[4] * wv_convert_low)))/delta_lambda
                        ...
                        if test:
                            halo_fescs.append(np.sum(stellar_lintr[ind_star:ind_star + nb_cur_stars] * stellar_fesc[0, ind_star:ind_star + nb_cur_stars]) / np.sum(stellar_lintr[ind_star:ind_star + nb_cur_stars]))
                            
                            print(stellar_fesc[iset, ind_star:ind_star + nb_cur_stars].min(), stellar_fesc[iset, ind_star:ind_star + nb_cur_stars].max(), stellar_fesc[iset, ind_star:ind_star + nb_cur_stars].mean())
            ...
            # print(halo_2e4k_gmass, halo_2e4k_gmass.max(), np.mean(halo_2e4k_gmass))
                ...
                if not test:
                    ...
                    fig,axs=plt.subplots(1,2, figsize=(10,10), sharey=True, width_ratios=[0.85, 0.15])
                    ...
                    axs[0].scatter(stellar_rho, stellar_fesc[0], c=np.log10(stellar_lintr), s=5, cmap='viridis')

                    #plot mean in bins
                    rho_bins = np.logspace(-28, -24, 8)
                    mean,bins,counts = binned_statistic(stellar_rho, stellar_fesc[0]*stellar_lintr, statistic='sum', bins=rho_bins)
                    sum,bins,counts = binned_statistic(stellar_rho, stellar_lintr, statistic='sum', bins=rho_bins)
                    mean = mean / sum

                    axs[0].plot(bins[:-1], mean, 'r--', label='mean')

                    axs[1].hist(stellar_fesc[0], bins=np.logspace(-10,1,60), orientation='horizontal')
                    axs[1].scatter(1., halo_fescs[-1], marker='*', color='r', s=100)
                    ...
                    print(len(stellar_rho), len(stellar_fesc[0]))
                    axs[0].scatter(stellar_taus, stellar_fesc[0], c=stellar_age, s=5, cmap='viridis')
                    axs[1].scatter(stellar_rho, np.exp(-stellar_taus), c=stellar_age, s=5, cmap='viridis')
                    ...
                    df = pd.DataFrame({'rho':stellar_rho.tolist(), 
                    ...
                    return(stellar_Z, stellar_age, sub_halo_tab, stellar_fluxes, stellar_mag, stellar_beta, stellar_fesc, sub_halo_tab["mass"] * Mp, stellar_lintr)
